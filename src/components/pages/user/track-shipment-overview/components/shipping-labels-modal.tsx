...
export default function ShippingLabelsModal({
  ...
  const handleDirectDownload = async (labelKey: string) => {
    console.log(labelKey, "label key");

    try {
      const url = `${Config.apiUrl}auth/download/labels/${labelKey}`;
      window.open(url, "_blank");
      // const response = await axios.get(
      //   `${Config.apiUrl}/auth/download/labels/${labelKey}`,
      //   {
      //     responseType: "blob",
      //   }
      // );
      ...
      // link.href = url;
      // link.setAttribute("download", `shipping-label-${labelKey}.pdf`);
                    ...
                    return (
                      <div key={label.id} className="border p-4 rounded-md">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                          ...
                          <div className="flex items-end">
                            <Button
                              onClick={() => handleDirectDownload(label.key)}
                              title={
                                isDownloadingKey === label.key
                                  ? "Downloading..."
                                  : "Download PDF"
                              }
                              variant="primary"
                              disabled={isDownloadingKey === label.key}
                            />
                          </div>
                        </div>
                      </div>
